...
if [ "$1" ]; then
	list=$(az account show --subscription $1 --query id --output tsv);
	echo "Subscription successfully loaded:" $1
else
...
az ad app update --id $(echo $pim | cut -d' ' -f1) --web-redirect-uris https://localhost:63222 --public-client-redirect-uris https://localhost:63222
...
az ad app permission grant --id $(echo $pim | cut -d' ' -f1) --api 00000003-0000-0000-c000-000000000000 --scope user_impersonation
